...
CREATE TABLE IF NOT EXISTS `pizzeria_inma_db`.`productos_tipo` (
  `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  `tipo` ENUM('Hamburguesas', 'Pizzas', 'Bebidas') NOT NULL DEFAULT 'Hamburguesas',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
AUTO_INCREMENT = 4
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `pizzeria_inma_db`.`productos_pizza` (
  ...
  CONSTRAINT `FK_productos_pizza_productos`
    FOREIGN KEY (`id_producto_tipo`)
    REFERENCES `pizzeria_inma_db`.`productos` (`id_tipo_producto`))
ENGINE = InnoDB
AUTO_INCREMENT = 12
DEFAULT CHARACTER SET = utf8;
